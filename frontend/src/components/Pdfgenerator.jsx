...
const Pdfgenerator = () => {
  ...
  return (
    <section
      id="pdf-generator"
      className="relative min-h-[90vh] w-full flex items-center justify-start bg-black text-white overflow-hidden px-20"
    >
      {/* 🔴 Gradient Overlay */}
      ...
      <div className="relative z-10 flex flex-col md:flex-row items-center justify-between max-w-7xl w-full gap-10">
        ...
        <div className="flex-1 text-left space-y-8">
          ...
          <button onClick={()=>navigate("/pdf")} className="border-2 border-white text-white px-8 py-3 rounded-full bg-transparent hover:bg-white hover:text-black font-semibold transition duration-300">
            Generate Notes
          </button>
        </div>
      </div>
       <div className=" absolute top-10 right-10 h-fit">
          <img src="/pdf.png" alt="PDF Illustration" className="w-200 h-200 object-contain opacity-90 mask"/>
        </div>
    </section>
  );
};
